    ...
    fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${this.channelId}&type=video&maxResults=10&publishedAfter=${dayjs().utc().subtract(8, "d").format("YYYY-MM-DD"+"T"+"HH:mm:ss.ms")}Z&key=${key}`)
      ...
      .then((json)=>{  
        ...
        json.items.forEach((element)=>{                                      
            ...
            this.list.push({videoId:element.id.videoId, videoTitle:videoTitle, videoThumbnail:videoThumbnail});
        });
      });      
  }
  api(key, daysAgo){      
...
const process1 = (list, listIcon, printLN, printUC, printA, key2)=>{
  ...
  fetch(`https://www.googleapis.com/youtube/v3/videos?part=snippet,liveStreamingDetails,status,contentDetails&id=${videoIds}&maxResults=50&key=${key2}`)
      .then((response)=>{
        console.log(`https://www.googleapis.com/youtube/v3/videos?part=snippet,liveStreamingDetails,status,contentDetails&id=${videoIds}&maxResults=50&key=${key2}`);
        return response.json();
      })
      .then((json)=>{           
        for(let i=0; i<json.items.length; i++){
          if(list.find((element)=>{if(element.videoId===json.items[i].id){return true}})){
            ...
            if(json.items[i].snippet.liveBroadcastContent==="live"){
              printLN.push({videoId:object.videoId, videoTitle:object.videoTitle, videoThumbnail:object.videoThumbnail, time:json.items[i].liveStreamingDetails.actualStartTime, icon:icon1[0], icon2:icon2});
            }

            if(json.items[i].snippet.liveBroadcastContent==="upcoming"){
              printUC.push({videoId:object.videoId, videoTitle:object.videoTitle, videoThumbnail:object.videoThumbnail, time:json.items[i].liveStreamingDetails.scheduledStartTime, time2:"time2", status:json.items[i].status.uploadStatus, icon:icon1[0], icon2:icon2});
            }
            ...
            if(json.items[i].snippet.liveBroadcastContent==="none"){
              ...
              printA.push({videoId:object.videoId, videoTitle:object.videoTitle, videoThumbnail:object.videoThumbnail, time:json.items[i].liveStreamingDetails.actualStartTime, time2:json.items[i].contentDetails.duration, time3:json.items[i].liveStreamingDetails.actualStartTime, icon:icon1[0], icon2:icon2});
            }
          }     
        }
      });
...
cron.schedule('40 * * * * *', () => {
  adjust1(ObjectData, listIcon, key2);
});

cron.schedule('50 * * * * *', () => {
  adjust2(ObjectData, listIcon, key2);
});
